...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $sendMail = ''; // Initialize the $sendMessage variable
        // Insert order items
        foreach ($items as $item) {
            ...
            // Subtract the number of yards from the products table
            $sqlSubtract = "UPDATE products SET yards = yards - ? WHERE productid = ?";
            $stmtSubtract = mysqli_prepare($conn, $sqlSubtract);
            mysqli_stmt_bind_param($stmtSubtract, "is", $product_yards, $product_id);
            mysqli_stmt_execute($stmtSubtract);
            mysqli_stmt_close($stmtSubtract);
