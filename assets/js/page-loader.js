function checkNetworkSpeed() {
  if ('connection' in navigator) {
    ...
    if (effectiveType === '2g' || effectiveType === 'slow-2g' || effectiveType === '3g') {
      showNotification3('⚠️ Your connection is slow. Some features might not work as expected.', 'info'); // Show notification
    }
  }
}
...
function updateConnectionStatus() {
  if (!navigator.onLine) {
    ...
    if (!offlineNotification) { // Ensure no duplicate offline notifications
      showNotification3('⚠️ You are offline. Please check your internet connection.', 'info', true);
    }
